...
def bellman_operator(env: GenericModel, value: np.ndarray, discount: float):
    ...
    for aa in range(env.action_dim):
        q_value[:, aa] = env.reward_matrix[:, aa] + discount * env.transition_matrix[
            aa
        ].dot(value)
    return q_value
...
def compute_transition_reward_policy(
    ...
    """
    transition_policy = np.zeros((env.state_dim, env.state_dim))
    reward_policy = np.zeros(env.state_dim)
    for aa in range(env.action_dim):
        ind: np.ndarray = (policy == aa).nonzero()[0]
        if ind.size > 0:
            transition_policy[ind, :] = env.transition_matrix[aa][ind, :].todense()
            reward_policy[ind] = env.reward_matrix[ind, aa]
